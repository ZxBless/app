...
import { Search, Pencil, Trash2, Calendar, Pickaxe, Hammer } from 'lucide-react-native';
...
export default function HistoryScreen() {
  ...
  return (
    <View style={styles.screen}>
      ...
      {summaries.length === 0 ? (
        <View style={styles.center}><Calendar size={48} color={colors.grayBorder} strokeWidth={2} /><Text style={styles.emptyText}>No hay registros{search ? ' que coincidan' : ''}.</Text></View>
      ) : (
        <FlatList data={summaries} keyExtractor={(item) => item.date} contentContainerStyle={{ padding: spacing.lg }} renderItem={({ item: summary }: { item: DaySummary }) => {
          ...
          return (
            <View style={styles.daySection}>
              <View style={styles.dayHeader}>
                ...
                <View style={styles.dayTotal}><Text style={styles.dayTotalValue}>{formatCurrency(summary.totalPayment)}</Text></View>
              </View>
              {dayRecords.map((rec) => (
                <TouchableOpacity key={rec.id} style={styles.recordCard} onPress={() => router.push(`/modal/new-record?editId=${rec.id}`)}>
                  <View style={styles.recordLeft}>
                    <View style={[styles.typeBadge, rec.workType === 'excavacion' ? styles.badgeExcavacion : styles.badgeMartillo]}>
                      {rec.workType === 'excavacion' ? <Pickaxe size={18} color={colors.black} strokeWidth={2.5} /> : <Hammer size={18} color={colors.black} strokeWidth={2.5} />}
                    </View>
                  ...
                  <View style={styles.recordRight}>
                    ...
                    <View style={styles.recordActions}>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => router.push(`/modal/new-record?editId=${rec.id}`)}><Pencil size={16} color={colors.grayText} strokeWidth={2.5} /></TouchableOpacity>
                      <TouchableOpacity style={styles.actionBtn} onPress={() => setDeleteId(rec.id)}><Trash2 size={16} color={colors.red} strokeWidth={2.5} /></TouchableOpacity>
                    </View>
                  </View>
                </TouchableOpacity>
              ))}
...
const styles = StyleSheet.create({
  ...
  searchBar: { flexDirection: 'row', alignItems: 'center', backgroundColor: colors.white, margin: spacing.lg, marginBottom: 0, borderRadius: radius.md, paddingHorizontal: spacing.md, paddingVertical: spacing.sm, borderWidth: 2, borderColor: colors.grayBorder },
  ...
  dayHeader: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', backgroundColor: colors.black, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.sm },
  dayDate: { fontSize: 18, fontWeight: '800', color: colors.yellow },
  ...
  recordCard: { flexDirection: 'row', backgroundColor: colors.white, borderRadius: radius.md, padding: spacing.md, marginBottom: spacing.sm, alignItems: 'center', justifyContent: 'space-between', shadowColor: colors.black, shadowOffset: { width: 0, height: 1 }, shadowOpacity: 0.08, shadowRadius: 3, elevation: 2 },
  ...
  badgeExcavacion: { backgroundColor: colors.yellow },
  badgeMartillo: { backgroundColor: colors.yellowLight },
  recordInfo: { flex: 1 },
  ...
  recordPayment: { fontSize: 16, fontWeight: '800', color: colors.yellowDark },
